...
def get_filters():
    ...
    while day not in days:
            print('You have entered invalid day')
            day = input('Which day? (Monday, Tuesday, Wednesday, Thursday, Friday, Saturday, or Sunday).').title()
    else:
        day = 'all'
...
def station_stats(df):
    ...
    print(f'The most frequent combination of start station and end station trip is: from {popular_trip.mode()[0]}.')
...
def display_raw_data(df):
    ...
    raw = input('Would you like to diplay raw data? (Yes or No)')
    if raw.lower() == 'yes':
        count = 0
        while True:
